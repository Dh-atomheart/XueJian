...
from ..rag.audit import audit_citations as _audit_citations
from ..rag.context import merge_parent_context as _merge_parent_context, packing_summary as _packing_summary, dedupe_and_pack_chunks
from ..rag.retriever import embedding_readiness_status, embed_optional_query, search_candidates_for_queries
from ..rag.rerank import rerank_evidence as _rerank_evidence, relevance_gate
from ..rag.trace import _lexical_status, build_rag_trace as _build_rag_trace
...
def retrieve_evidence(tool_input: RetrieveEvidenceInput) -> RetrieveEvidenceOutput:
    ...
    readiness_status = embedding_readiness_status(
        host,
    ...
    if readiness_status == "ready":
        query_embedding = embed_optional_query(host, active_profile, query_text)
...
def rerank_evidence(tool_input: RerankEvidenceInput) -> RerankEvidenceOutput:
    chunks, rerank_summary = _rerank_evidence(
        tool_input.chunks,
        tool_input.question,
...
def grade_retrieval_relevance(
    ...
    decision, gate_summary = relevance_gate(
        tool_input.chunks,
...
def pack_context(tool_input: PackContextInput) -> PackContextOutput:
    packed_chunks = dedupe_and_pack_chunks(tool_input.chunks)
    packing_summary = _packing_summary(packed_chunks, tool_input.expanded_contexts)
...
def audit_citations(tool_input: AuditCitationsInput) -> AuditCitationsOutput:
    citations, removed_ids, audit_status = _audit_citations(
