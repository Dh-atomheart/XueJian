    ...
    <div className="flex h-full gap-4 p-4">
      ...
      <Panel className="flex w-80 shrink-0 flex-col gap-3 overflow-y-auto">
        ...
        <div className="flex flex-col gap-2 border-b pb-3">
          <p className="text-sm text-ink-muted">选择文档构建图谱</p>
          <select
            multiple
            className="h-20 rounded border border-line-soft bg-paper-muted p-1 text-sm text-ink"
        ...
        {nodesLoading ? (
          ...
          <p className="text-sm text-ink-muted">暂无节点，请先构建图谱</p>
        ) : (
        ...
        {buildRuns.length > 0 && (
          <div className="mt-auto border-t pt-2">
            <p className="text-xs font-medium text-ink-muted">构建记录</p>
            {buildRuns.slice(0, 3).map((run) => (
              <div key={run.id} className="text-xs text-ink-muted">
        ...
        {selectedNode ? (
          <>
            ...
            <div className="border-b pb-3">
              ...
              {sources.length === 0 ? (
                <p className="text-sm italic text-ink-muted">无已知来源</p>
              ) : (
                <ul className="text-sm">
                  {sources.map((srcId) => {
                    const doc = documents.find((d) => d.id === srcId)
                    return (
                      <li key={srcId} className="text-ink-muted">
                        {doc ? doc.title : srcId}
                      </li>
                    )
                  })}
            ...
            <div className="border-b pb-3">
              ...
              {edges.length === 0 ? (
                <p className="text-sm italic text-ink-muted">暂无关系</p>
              ) : (
...
function EdgeRow({
  ...
  return (
    <tr className="border-b last:border-0">
      ...
      <td className="py-1">
        <button
          className="text-ink underline decoration-ink-soft"
