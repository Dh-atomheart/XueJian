...
export function GraphPage() {
  ...
  useEffect(() => {
    ...
    documents.forEach((doc, i) => {
      ...
      nodes.push({
        id: 'doc-' + doc.id, label: doc.name.replace('.pdf', ''), type: 'document',
        x: cx + Math.cos(angle) * 150 + (Math.random() - 0.5) * 40,
        y: cy + Math.sin(angle) * 150 + (Math.random() - 0.5) * 40,
        vx: 0, vy: 0, color: '#6b7280', radius: 20,
      })
    })
    ...
    groups.forEach((group, i) => {
      ...
      nodes.push({
        id: 'grp-' + group.id, label: group.name, type: 'concept',
        x: cx + Math.cos(angle) * 100 + (Math.random() - 0.5) * 30,
        y: cy + Math.sin(angle) * 100 + (Math.random() - 0.5) * 30,
        vx: 0, vy: 0,
        color: group.color === 'yellow' ? '#eab308' : group.color === 'blue' ? '#3b82f6' : '#22c55e',
        radius: 16,
      })
    })
    ...
    flashcards.slice(0, 12).forEach((card, i) => {
      ...
      nodes.push({
        id: 'card-' + card.id, label: card.front.slice(0, 12), type: 'card',
        x: cx + Math.cos(angle) * 220 + (Math.random() - 0.5) * 40,
        y: cy + Math.sin(angle) * 220 + (Math.random() - 0.5) * 40,
        vx: 0, vy: 0, color: '#a3a3a3', radius: 10,
      })
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const clicked = nodesRef.current.find((n) => Math.sqrt((n.x - x) ** 2 + (n.y - y) ** 2) < n.radius + 4)
  ...
  const handleCanvasMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (!rect) return
    const x = e.clientX - rect.left, y = e.clientY - rect.top
    const hovered = nodesRef.current.find((n) => Math.sqrt((n.x - x) ** 2 + (n.y - y) ** 2) < n.radius + 4)
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in">
      <div className="px-6 py-4 border-b border-line-soft">
        <p className="text-xs tracking-[0.3em] text-ink-muted uppercase mb-1 font-ui">Knowledge Graph</p>
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute bottom-4 left-4 flex gap-4 text-xs text-ink-muted bg-paper-base/80 backdrop-blur-sm rounded-lg px-3 py-2 border border-line-soft/60">
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-gray-500 inline-block" /> 文档</span>
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-yellow-500 inline-block" /> 概念</span>
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-gray-400 inline-block" /> 卡片</span>
        </div>
        ...
        {selectedNode && (
          <div className="absolute top-4 right-4 w-56">
            <SketchCard className="text-sm">
              ...
              <p className="text-xs text-ink-muted capitalize">{selectedNode.type === 'document' ? '文档' : selectedNode.type === 'concept' ? '概念' : '卡片'}</p>
            </SketchCard>
          </div>
        )}
      </div>
