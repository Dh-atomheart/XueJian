...
自 2026-04-24 起，卡片系统主流程改为：

```text
文档就绪 -> Agent 生成、校验、去重与质量过滤 -> 正式 cards 自动落库 -> 用户对正式卡片增删查改 -> 复习调度
```

本修订覆盖本文档后续旧段落中关于“候选卡片人工审核、接受/拒绝、waiting_confirmation、resume、finalize 后再入库”的描述。`card_candidates` 可作为兼容遗留表继续存在，但不再是卡片生成主路径的用户可见状态机。

当前职责边界：

- Agent 负责生成、处理、去重和质量过滤；低质量结果直接丢弃。
- 系统只把通过质量门槛的结果写入正式 `cards`。
- 用户只对正式卡片拥有增删查改能力。
- `CardStudioPage` 是 Agent 生成入口和正式卡片 CRUD 工作台，不再是候选审批台。
