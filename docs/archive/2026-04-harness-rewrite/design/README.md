...
本目录用于沉淀学笺桌面端的默认设计系统与页面规范，服务对象包括产品、设计、前端实现和后续 AI 协作。文档目标不是输出抽象概念，而是把 `spec.md` 已锁定的视觉基线细化成可执行规则。
...
- `spec.md` 已锁定：默认视觉基线、关键 Token、桌面多栏结构、阶段边界
- `docs/design/*` 细化：字体用法、组件状态、动效边界、页面层级、异常态和交互规则

若本目录与 `spec.md` 冲突，以 `spec.md` 为准，并回补修订本目录。

## 7. 更新原则

- 先改上位规范，再改设计文档，不反向覆盖 `spec.md`
