...
> 历史说明：`docs/archive/` 仅用于追溯历史方案，不代表当前 AI/BYOK 页面准入策略。现行规则以活动文档为准：缺少 AI/BYOK 配置时页面仍可浏览，只有真正发起 AI 调用的动作按能力降级。
...
- [ai-orchestration.md](./references/ai-orchestration.md)：AI 编排架构决策、运行边界与跨端策略
- `一、项目概述`：愿景、用户、核心故事
- `二、功能规范`：按阶段展开的产品规格
...
1. 用户点击"上传文档"按钮，选择本地 PDF 文件
2. Rust Host 将文件落盘至应用数据目录，创建 `Document` 记录（`status='uploading'`）
3. Rust 调用 Python sidecar 的 **Docling + PyMuPDF 解析管线**，产出结构化页面信息（hierarchy、bbox、text_quote、quote_hash）
4. Python 解析完毕后回写 Rust：创建 `DocumentChunk`（FTS5 索引）与 `DocumentAnchor`（含 hierarchyPath + bbox + quoteHash）
5. `Document.status` 更新为 `'parsed'`，前端显示文档预览
6. 用户确认后进入卡片生成流程

**解析管线说明：**

- **Docling**：负责文档结构识别（标题层级、段落、表格、图表说明），产出带 `hierarchyPath` 的块级结构
- **PyMuPDF**：负责精确 bbox 坐标提取、文本引用（quote）与页面元信息
- **PDF.js**：仅用于前端 UI 渲染与文本选择热区，不承担解析与锚点建模
- 两套解析互补：Docling 提供语义层级，PyMuPDF 提供坐标精度
...
DocumentAnchor {
  ...
  hierarchyPath: string[]   // e.g. ['Chapter 3', 'Section 3.2', 'Paragraph 4']
  quoteHash: string         // SHA-256 of normalized textQuote
}
```

**技术约束：**

- 文档解析由 Python sidecar（Docling + PyMuPDF）完成，PDF.js 仅负责前端渲染
- 文档导入失败时保留错误状态，不写入半成品卡片
- 锚点至少包含 page + quote + rects + hierarchyPath + quoteHash 五重信息，避免后续贴笺错位
- Docling 模型首次运行需下载，应用需展示下载进度并支持离线包预装
...
1. 将 PDF 文本按章节/段落分块（复用 Docling 产出的 hierarchyPath 做智能边界）
2. 通过 Python sidecar 调用 LLM，使用 **PydanticAI 强类型输出**（`CardDraft` 模型）生成卡片候选
3. 候选卡片写入 `CardCandidate`，保留来源锚点、置信度与去重键
4. 用户对候选执行接受、编辑、删除、批量确认
5. 确认后保存为正式 `Card`，同时生成稳定 `exportGuid`

**结构化输出说明：**

- LLM 输出由 **PydanticAI** 的 `CardDraft` 模型严格校验，不接受自由文本拼装
- `CardDraft` 字段包括：`title`、`front`、`back`、`cardType`、`tags`、`anchorRef`、`confidence`
- 校验失败时触发自动重试（最多 2 次），仍失败则标记该块为 `generation_error` 跳过
- Prompt 模板由 Python 侧 `PresetWorkflow` 管理，支持用户在设置中微调
...
CardCandidate {
  ...
  front: string
  back: string
  cardType: 'qa' | 'cloze' | 'fact'  // default 'qa'
  tags: string[]
  confidence: number
  dedupeKey: string
}
...
Card {
  ...
  documentId: string
  anchorId?: string
  groupId?: string       // semantic group suggested by LLM
  clusterId?: string     // user-managed cluster/deck
  exportGuid: string     // stable Anki GUID, generated once at creation
  sourcePage: number
...
- 支持批量确认、批量删除、批量修改标签
- 支持 CSV 导出（纯文本）
- 支持 `.apkg` 导出（由 Python sidecar 的 **genanki** 生成原生 Anki 包，使用 `exportGuid` 保证重复导出时卡片 GUID 稳定）
...
- OpenAI（走 OpenAI 原生协议）
- Anthropic（走 **Claude 原生 API**，不走 OpenAI 兼容路径）
- 自定义端点（走 OpenAI 兼容协议，用户自行填写 baseUrl）

**协议路由规则：**

- `provider='openai'` → OpenAI SDK / LiteLLM openai 路由
- `provider='anthropic'` → Anthropic 原生 Messages API / LiteLLM anthropic 路由
- `provider='custom'` → OpenAI 兼容端点（用户提供 baseUrl）
...
- `ToolGateway` 负责文档解析、检索、导出（含 genanki `.apkg`）、音频生成和数据库写入等受控工具调用
- `Python Orchestration Service` 负责预设工作流编排、Prompt 组装和状态推进
- 详细运行边界以 [ai-orchestration.md](./references/ai-orchestration.md) 为准

**LLM 适配层实现：LiteLLM + PydanticAI**

- **LiteLLM**：作为 `ModelGateway` 的 provider 适配层，统一 OpenAI / Anthropic / Custom 端点协议
  - Rust Host 负责密钥注入（从 Stronghold 读取后注入 HTTP 请求头），LiteLLM 不持久化密钥
  - LiteLLM 运行在 Python sidecar 进程中，Rust 通过 Orchestration Protocol 调用
  - 成本统计由 LiteLLM callback 产出，Rust 侧聚合落库
- **PydanticAI**：作为结构化输出校验层
  - `CardDraft`、`RagAnswer`、`Citation`、`WorkflowEvent` 等关键输出类型通过 Pydantic 模型强约束
  - 校验失败触发自动重试（retry_count ≤ 2），超限后降级为 error 事件
- **协议路由**：`provider='openai'` → LiteLLM openai 路由；`provider='anthropic'` → LiteLLM anthropic 路由（原生 Messages API）；`provider='custom'` → LiteLLM openai-compatible 路由

**Python sidecar MVP 依赖：**

- `langchain` / `langchain-openai` / `langchain-anthropic`（PresetWorkflow 编排）
- `litellm`（provider 适配）
- `pydantic` / `pydantic-ai`（结构化输出校验）
- `docling` / `pymupdf`（文档解析管线）
- `genanki`（Anki .apkg 导出）
- `httpx`（Host 通信）
...
DocumentAnchor {
  ...
  textQuote: string
  rects: { x: number; y: number; width: number; height: number }[]
  hierarchyPath: string[]   // from Docling: document structure hierarchy
  quoteHash: string         // SHA-256 of normalized textQuote
}
...
CREATE TABLE document_anchors (
    ...
    rects JSON,
    hierarchy_path JSON,              -- from Docling: e.g. ["Chapter 3", "Section 3.2"]
    quote_hash TEXT NOT NULL,          -- SHA-256 of normalized text_quote
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE cards (
    ...
    cluster_id TEXT,                    -- user-managed cluster/deck
    document_id TEXT REFERENCES documents(id),
    anchor_id TEXT REFERENCES document_anchors(id),
    card_type TEXT DEFAULT 'qa',        -- 'qa' | 'cloze' | 'fact'
    export_guid TEXT NOT NULL UNIQUE,   -- stable Anki GUID, generated once at creation
    front TEXT NOT NULL,
    back TEXT NOT NULL,
    source_page INTEGER,
...
**Python Orchestration Service 依赖：**
...
- [ ] 建立 `docs/exec-plans/` 脚手架与 `_template.md`
- [ ] 配置Tailwind CSS + Radix Primitives + 默认设计Token + Rough.js 点缀封装
- [ ] 接入字体资源、纸感纹理、手绘线条规则与桌面多栏壳层
- [ ] 实现SQLite数据库初始化、迁移（含 0003 Card schema 扩展 + 0004 Anchor provenance）与FTS5索引
- [ ] 实现Stronghold密钥存储和基础Repository层
- [ ] 编写数据层单元测试

**第2周：Host 网关与 Python 编排服务**

- [ ] 实现 `ModelGateway / ToolGateway`（LiteLLM 作为 provider 适配层）
- [ ] 实现 `Python Orchestration Service` 基础设施与健康检查
- [ ] 集成 LiteLLM + PydanticAI：provider 路由、结构化输出校验、成本 callback
...
- [ ] 实现PDF上传功能（Rust 落盘 + 状态管理）
- [ ] 集成 Docling + PyMuPDF 解析管线（Python sidecar）：产出 hierarchyPath / bbox / quoteHash
- [ ] 实现PDF.js前端渲染与文本选择热区
- [ ] 实现文档转卡片工作流（PydanticAI CardDraft 输出）
- [ ] 实现 genanki `.apkg` 导出（Python sidecar）
- [ ] 实现FSRS调度算法（ts-fsrs）
- [ ] 实现贴笺展示功能
- [ ] 实现卡片学习界面与基础学习统计
- [ ] 编写相关测试
...
- [ ] 实现BYOK配置界面与模型连接测试（Anthropic 原生 API / OpenAI / Custom）
- [ ] 整合统计、导出（CSV + `.apkg`）、任务恢复与错误处理
- [ ] Card schema 迁移 0003/0004 验证（空库 + 既有库）
- [ ] Rough.js 点缀视觉验收（空状态、卡片边框、分隔线）
- [ ] 编写E2E测试
- [ ] Bug修复与优化
- [ ] 打包测试
...
| Docling 首次冷启动  | 模型下载体积大、首次慢     | 中   | 提供离线包预装、显示进度条、支持跳过等待后台下载 |
| LiteLLM 版本兼容    | 新版本破坏 provider 路由   | 中   | 锁定 minor 版本、CI 跑兼容性冒烟测试        |
| genanki GUID 策略   | 定死后难回滚               | 低   | exportGuid 在 Card 首创时生成并永久保存      |
| PydanticAI 校验过严 | 模型输出微变即失败         | 中   | 自动重试 2 次、降级到宽松解析器、记录错误样本 |
...
| LiteLLM        | provider 路由不完整或高级能力缺失 | 仅承诺基础 completion/stream；高级能力（extended thinking等）V2+回填 |
| Docling        | 文档结构识别准确率有限、模型大 | PyMuPDF 兜底坐标层；离线包 + 进度提示    |
| PydanticAI     | 库更新频繁、接口不稳定      | 锁定版本、封装适配层、保留降级到原始 JSON 解析 |
| genanki        | Anki 格式兼容性（新版本字段变更） | 导出前跑 GUID 稳定性回归测试              |
| SQLite FTS5    | 仅关键词检索，语义能力有限  | V2引入 sqlite-vec 与混合检索              |
...
_创建日期：2026-04-15_
_最后更新：2026-04-19_
_更新内容：研究对齐修订 — 基于 `docs/总框架决策的研究.md` 深度调研，并入 Docling+PyMuPDF 解析管线、LiteLLM+PydanticAI 模型网关、Card schema 扩展（title/groupId/clusterId/exportGuid/cardType）、genanki .apkg 导出、Anthropic 原生 API 优先、Rough.js 点缀；删除前端 @langchain/* 依赖；新增 Python sidecar 依赖清单；更新 §2.2.2 / §2.2.3 / §2.2.6 / §3.1.6 / §4.2 / §4.4 / §6.1 / §6.2 / §8。对应 exec-plan: `docs/exec-plans/active/m-research-alignment.md`_
